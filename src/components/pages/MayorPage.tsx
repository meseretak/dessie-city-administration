        ...
        <div className="max-w-6xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={staggerContainer}>
            <motion.div variants={fadeInUp} className="text-center mb-12">
              ...
              <h2 className="gov-section-title text-2xl md:text-3xl font-bold text-[#0d4a28] inline-block">ADMINISTRATION &amp; STRUCTURE</h2>
              <p className="text-muted-foreground mt-3 max-w-2xl mx-auto text-sm">Click on any item to view full information.</p>
            </motion.div>

            <motion.div variants={fadeInUp} className="flex flex-col items-center">
              {/* Level 1: Mayor */}
              <div className="w-full max-w-xs">
                <OrgNode icon={orgStructure.mayor.icon} title={orgStructure.mayor.title} subtitle={orgStructure.mayor.name} color="#0d4a28" size="lg" highlight clickable onClick={() => setSelectedOrg({ title: orgStructure.mayor.title, bio: mayorInfo.bio, photo: mayorInfo.photo, name: mayorInfo.name, role: mayorInfo.title, email: mayorInfo.email, phone: mayorInfo.phone, social: mayorInfo.social })} />
              </div>
              <VLine />
              {/* Level 2: Deputy */}
              <div className="w-full max-w-xs">
                <OrgNode icon={orgStructure.deputy.icon} title={orgStructure.deputy.title} subtitle={orgStructure.deputy.name} color="#1a6b3c" size="md" clickable onClick={() => setSelectedOrg({ title: orgStructure.deputy.title, bio: deputyInfo.bio, photo: deputyInfo.photo, name: deputyInfo.name, role: deputyInfo.title, email: deputyInfo.email, phone: deputyInfo.phone, social: deputyInfo.social })} />
              </div>
              <VLine />
              ...
              <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {orgStructure.departments.map((dept, i) => {
                  ...
                  const member = cabinetMembers.find(m => m.title.includes(dept.title.split(' ')[0]))
                  return (
                    <motion.div
                      key={dept.title}
              ...
              <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-3">
                {orgStructure.subCities.map((sc, i) => (
                  <motion.div
                    ...
                    className="px-3 py-2.5 rounded-xl border border-[#e2e8e0] bg-[#f8faf8] hover:bg-white hover:shadow-sm transition-all text-center"
                  >
